...
import { api } from "../api";
import type { FinancialDailyProduction } from "../types";
...
export default function FinancialProductivityPage() {
  ...
  const load = useCallback(() => {
    setErr(null);
    return api.financial.listProduction(projectId).then(setRows);
  }, [projectId]);
  ...
  const sorted = [...rows].sort((a, b) =>
    a.day < b.day ? 1 : a.day > b.day ? -1 : a.team_type.localeCompare(b.team_type)
  );
    ...
    <div className="space-y-8">
      <div className="rounded-2xl border border-emerald-500/20 bg-gradient-to-br from-emerald-500/[0.06] to-transparent p-6 md:p-8">
        ...
        <p className="mt-2 max-w-3xl text-sm text-slate-400">
          Valor produzido (R$) por dia e tipo de equipe, com campo de observações quando necessário.
        </p>
      </div>
          ...
          <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
            ...
            <div>
              ...
              <input
                ...
                placeholder="Mesmo texto do planejamento"
              />
            </div>
            <div>
              ...
              {sorted.length === 0 ? (
                <tr>
                  <td colSpan={5} className="px-6 py-12 text-center text-slate-500">
                    Nenhum lançamento. Use «Novo lançamento».
                  </td>
                </tr>
              ) : (
                sorted.map((p) => (
